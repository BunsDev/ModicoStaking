...
async function submitSources(bre, solcInputsPath, config) {
    config = config || {};
    ...
    async function submit(name, useSolcInput) {
        ...
        if (result === "failure") {
            if (!useSolcInput && fallbackOnSolcInput) {
                logInfo("Falling back on solcInput. etherscan seems to sometime require full solc-input with all source files, even though this should not be needed");
                await submit(name, true);
            }
            else {
                logInfo("Etherscan sometime fails to verify when only metadata sources are givem, You can add the option --solc-input to try with full solc-input sources. This will include all contract source in the etherscan result, even the one not relevant to the contract being verified");
            }
        }
    }
