...
class DeploymentsManager {
        ...
        const partialExtension = {
            ...
            getArtifact: async (contractName) => {
                ...
                try {
                    ...
                    for (const importPath of importPaths) {
                        try {
                            artifact = await plugins_1.readArtifact(importPath, contractName);
                            if (artifact) {
                                return artifact;
                            }
                        }
                        catch (ee) {
                            lastError = new Error(`cannot get artifact ${contractName} at ${importPath}`);
                        }
                    }
                }
                if (!artifact) {
            ...
            getArtifactSync: (contractName) => {
                ...
                try {
                    ...
                    for (const importPath of importPaths) {
                        try {
                            artifact = plugins_1.readArtifactSync(importPath, contractName);
                            if (artifact) {
                                return artifact;
                            }
                        }
    ...
    async dealWithPendingTransactions() {
        ...
        const pendingTxPath = path_1.default.join(this.deploymentsPath, this.env.network.name, ".pendingTransactions");
        try {
    ...
    async onPendingTx(tx, name, deployment) {
        if (this.db.writeDeploymentsToFiles && this.db.savePendingTx) {
            const chainId = await utils_1.getChainId(this.env);
            const deployFolderPath = path_1.default.join(this.deploymentsPath, this.env.network.name);
    ...
    async loadDeployments(chainIdExpected = true) {
        ...
        try {
            ...
            migrations = JSON.parse(fs_extra_1.default
                .readFileSync(path_1.default.join(this.deploymentsPath, networkName, ".migrations.json"))
                .toString());
        }
        catch (e) { }
        this.db.migrations = migrations;
        // console.log({ migrations: this.db.migrations });
        utils_1.addDeployments(this.db, this.deploymentsPath, networkName, chainId);
    ...
    async getSolcInputPath() {
        const chainId = await utils_1.getChainId(this.env);
        return path_1.default.join(this.deploymentsPath, this.env.network.name, "solcInputs");
    }
    async saveDeployment(name, deployment) {
        ...
        const filepath = path_1.default.join(this.deploymentsPath, this.env.network.name, name + ".json");
        ...
        if (toSave) {
            ...
            if (deployment.solcInputHash) {
                const solcInputsFolderpath = path_1.default.join(this.deploymentsPath, this.env.network.name, "solcInputs");
    ...
    async export(options) {
        ...
        try {
            chainId = fs_extra_1.default
                .readFileSync(path_1.default.join(this.deploymentsPath, this.env.network.name, ".chainId"))
                .toString();
        }
        catch (e) { }
        if (!chainId) {
    ...
    getImportPaths() {
        ...
        if (externalImports) {
            if (typeof externalImports === "string") {
                importPaths.push(path_1.default.join(this.env.config.paths.root, externalImports));
            }
            else {
                importPaths = importPaths.concat(externalImports.map(v => path_1.default.join(this.env.config.paths.root, v)));
            }
        }
        return importPaths;
    }
